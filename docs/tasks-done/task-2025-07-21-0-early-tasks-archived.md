...
- [x] Consider using an appropriate UI framework and tailwind. I'd prefer one with no external deps like shadcn, but there may be better options. We only really need UI components like this for the window "chrome" and settings etc. We'll probably have to hand-write CSS for the markdown editor to make it really beautiful. There may be better options for our use case than shadcn?
...
- [x] Check Tauri is configured in the most optimal way for nice-looking macos apps
    - [x] Transparent title bar (<https://v2.tauri.app/learn/window-customization/#macos-transparent-titlebar-with-custom-window-background-color>)
    - [x] Sensible window menu with Open Project and the minimum usual stuff you'd expect
...
- [x] Refine frontmatter sidebar panel UI
    ...
    - [x] Ensure the switches work properly with the labels - the labels for these should not be above the input but next to them like in most settings apps.
    - [ ] The two textarea do not expand vertically with their content. I think this should be the default behavior of shadcn's <Textarea> so let's work out what's preventing it working.
    ...
    - [x] Platform is an enum field in the astro schema. So it should render a dropdown not a textbox. The dropdown should have a blank option which removes the property (same behavior as blank fields for other things.)
...
- [x] Make the frontmatter panel resizable (using the shadcn `resizable` component maybe?) Should have sensible min and max widths.
...
- [x] Left sidebar UI improvements
    - [x] Completely rebuild the left sidebar with shadcn's components (see <https://ui.shadcn.com/docs/components/sidebar>). The sidebar header "Collections" should only say "Collections" when not inside a collection. Once the user has clicked into a collection it should use the name of the collection, capitalized. Eg "Articles" or "Notes". When inside a collection there should be a back arrow which takes you back up to the collections list. We can then remove everything bar the list of items in the collection from the scrollable area of the sidebar. We can maybe use Shadcn's sidebar patterns to make this UI work a bit better.
...
- [x] Add ability to rename files in the FileList. This needs to work as seamlessly and easily as possible so it can be done quickly.
...
- [x] Comprehensively review our approach to parsing `content.config.js` - I think we currently use RegEx, but if we're able to execute JS/TS in the compiled Tauri app it may be possible to use `zod` to read and understand the schema in a more robust/efficient/safe way. Could we use the md/mdx files and zod schemas to create our own typesafe objects as representations of frontmatter/schema etc, which our UI components can read? This would probably involve the the rust backend talking more with the TS front-end etc. It may not be worth the effort to do this.
...
- [x] Comprehensively review and rewrite `CLAUDE.md` with clear descriptions of the current design patterns, architecture, technology, examples, npm commands etc. Add instructions to check the docs when needed (with the appropriate Context7 tool calls to do so). Make sure CLAUDE.md is the best it can be. Also look for opportunities to gently optimize for token use without affecting its effectiveness.
...
- [x] **MANUAL WORK FOR DANNY** - Codify iA Writer style in `/docs/ia-writer-ui.md` (See Figma)
    ...
    - [x] Recreate views in Figma with a sensible base font-size -> identify all variables, breakpoints etc.
...
- [ ] Create theme for editor based on `/docs/ia-writer-ui.md` and current best practice for codemirror
    - [x] Set up reusable editor colour palette variables (see `/docs/ia-writer-ui.md`) in a way which make it easy to add a dark mode later. Set editor background colour etc.
    ...
    - [x] Add MD syntax highlighting colours for all other elements (links, images, footnotes, strikethrough, inline code, code blocks, blockquote etc.)
    - [x] Style blockquote nicely
...
**Phase 4.1 - Polish, Resilience & Usability**
...
- [x] Review EditorView.tsx. It is now a HUGE file and me must be able to extract a lot of the functionality into helpers, utils, other React components etc? Think like an expert front-end architect and plan a detailed refactor to separate things. Be careful not to break or alter the functionality while doing this.
    - [x] Bugs from refactor:
        ...
        - [x] The menubar menu items for bold, italic, add link and all the heading/paragraph ones should be disabled unless a markdown file is open and focussed in the editor window. They are always enabled.
        - [x] Remove any unnecessary comments and logging (but leave those which are helpful for understanding how things work in the future)
...
- [x] Path overrides pane allows user to optionally override Astro defaults paths for various things. Will need to update all code that uses these paths in the appropriate places. The defaults should be hard-coded as fallbacks in case there are no saved user preferences. These paths should be relative to the project root so they still work if a user moves their whole astro repo and reopens it.
